...
func (st *Set) Add(v string) { (*st)[v] = true }

func (st *Set) Exist(v string) bool { return (*st)[v] }
...
func solve(s string) string {
	var (
		set = NewSet(strings.Split(s, "")...)
	)

	if len(set.SetKeys()) == len(s) {
		return yes
	} else {
		return no
	}
