...
func solve(S string) interface{} {
	...
	// 	for i := 0; i < len(S); i++ {
	// 		res = append(res, strconv.Itoa(ans[i]), string(S[i]))
	// 	}
	// 	res = append(res, strconv.Itoa(ans[len(ans)-1]))

	// 	fmt.Println(strings.Join(res, " "))
	// }()

	return func(sl []int) (res int) {
		for _, v := range sl {
			res += v
		}
		return
...
func check(S string, ans []int) (string, bool) {
	var (
		res []string
		chk = true
	)
...
func solve2(S string) interface{} {
	...
	fmt.Println(check(S, ans))

	for i := len(S) - 1; 0 <= i; i-- {
		if S[i] == '>' {
			ans[i] = max_int([]int{ans[i], ans[i+1] + 1})
		}
	}
