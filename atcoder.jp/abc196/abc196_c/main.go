...
func solve2(N int64) interface{} {
	for i := int64(1); ; i++ {
		target, _ := strconv.ParseInt(strconv.FormatInt(i, 10)+strconv.FormatInt(i, 10), 10, 64)
		if target > N {
			return i - 1
		}
	}
...
func solve(N int64) interface{} {
	...
	for i := 1; ; i++ {
		if i*2 != len(s) {
			ans += PowInt(10, i-1) * (10 - 1)
			debug.Println(PowInt(10, i-1) * (10 - 1))
		} else {
			top, _ := strconv.ParseInt(s[:i], 10, 64)

			if top != PowInt64(10, i-1) {
				ans += int(top - PowInt64(10, i-1) + 1)
			}
			break
		}
		debug.Println(ans)
